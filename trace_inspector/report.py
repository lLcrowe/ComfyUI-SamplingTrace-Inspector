...
def render_markdown(run: dict[str, Any], steps: list[dict[str, Any]]) -> str:
    ...
    if notes:
        for note in notes:
            ...
            lines.append(f"- **{note.get('label', 'observation')}** `{timestamp}` — {text}")
    else:
...
def render_html(run: dict[str, Any], steps: list[dict[str, Any]]) -> str:
    ...
    if notes:
        note_cards = "".join(
            '<article class="note">'
            f'<div><b>{html.escape(str(note.get("label", "observation")))}</b> '
...
def render_comparison_markdown(comparison: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "## Runs",
        "",
        "| Side | Run | Prompt | Label | Workflow hash | Requested sampler | Actual sampler | Steps |",
        "|---|---|---|---|---|---|---|---:|",
        f"| A | `{left.get('runId', '')}` | `{left.get('promptId') or '—'}` | {left.get('label') or '—'} | `{workflow.get('leftHash', '')}` | `{(runtime.get('left') or {}).get('requestedSampler', '')}` | `{(runtime.get('left') or {}).get('actualSampler', '')}` | {comparison.get('leftStepCount', 0)} |",
        f"| B | `{right.get('runId', '')}` | `{right.get('promptId') or '—'}` | {right.get('label') or '—'} | `{workflow.get('rightHash', '')}` | `{(runtime.get('right') or {}).get('requestedSampler', '')}` | `{(runtime.get('right') or {}).get('actualSampler', '')}` | {comparison.get('rightStepCount', 0)} |",
        "",
        "## Workflow relation",
        "",
